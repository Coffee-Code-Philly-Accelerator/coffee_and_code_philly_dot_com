...
import getIcon from "components/icons/icon";

const HighlightedText = tw.span`text-primary-600`;
const Heading = tw(SectionHeading)`text-primary-600 text-xl mb-5`;
const Posts = tw.div`flex flex-wrap border-2 rounded-4xl p-10 md:p-12 bg-gray-100 bg-opacity-10`;
const Info = tw.div`px-3 py-2 border-t-2 border-gray-300 rounded-lg bg-white rounded-t-none`;
const Description = tw.div`my-2 font-semibold`;
...
const AcceleratorCard = styled.div`
  ...
  ${RightTopDiv}, ${RightBottomDiv} {
    ${tw`h-20 w-full p-4 rounded-4xl bg-white text-center flex justify-center items-center font-semibold`}/* Right side stacked divs with 50% height each */
  }
  button {
    ${tw`w-full bg-primary-500 text-white font-bold mr-2 my-4 sm:my-2 rounded-full py-4 flex items-center justify-center sm:w-40 sm:leading-none focus:outline-none hover:bg-primary-900 transition duration-300 ml-3`}
  }
`;
                  ...
                  <Info>
                    ...
                    <SVGContainer>
                      {project.technologies &&
                        project.technologies.map((t, techIndex) => getIcon(t))}
                    </SVGContainer>
                  </Info>
                </Post>
              </ProjectContainer>
